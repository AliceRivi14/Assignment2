...
* `void Callback(const sensor_msgs::LaserScan::ConstPtr& msg)`:
    ...
    The request of the service `char input` is sent to the server (Control node) by the client (UI node). In the server the request is received and the velocity is modified consequently. The value of acceleration is assigned to service response `float32 acc`.
